...
def makeQueries(columnNames,coefficients, yIntercept):
	'''
...
def scaling(x):
	...
	x = numpy.asarray(x).T.tolist() # transpose
	...
	return numpy.asarray(x).T.tolist() # return the transpose
...
def main():
	...
	print("Final slope(s): ")
	for columnName, coefficient in zip(columnNames, coefficients):
		print(columnName, ":", coefficient)
